...
class ApiClientController extends CI_Controller
{
    ...
    {
        // Get the API key from the POST request (submitted by the user)
        $api_key = $this->input->post('api_key');

        // Check if API key is provided
        if (!$api_key) {
            // If no API key is provided, redirect to error page
            $this->session->set_flashdata('error', 'API Key is required.');
            redirect('client/api_key_form');
        }
